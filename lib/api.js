...
let API_BASE_URL = 'http://16.176.194.83/api'; // primary (expects reverse proxy / opened port 80)
...
export const detectApiBaseUrl = async () => {
  ...
  const tryHost = async (host) => {
    // Attempt no-port (80/443 via possible reverse proxy), then explicit :5000
    const variants = [
    ...
    for (const v of variants) {
      ...
      const timeoutId = setTimeout(() => controller.abort(), 1800); // slightly shorter per variant
      try {
  ...
  async testConnection() {
    ...
    for (const h of healthUrls) {
      try {
        ...
        console.warn(`⚠️ Health probe failed for ${h}:`, error?.message || String(error));
      }
    }
